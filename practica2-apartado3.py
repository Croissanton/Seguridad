...
class AES_CIPHER_CBC:
    ...
    def cifrar(self, cadena, IV):
        ...
        cipherCBC = AES.new(self.key, AES.MODE_CBC, IV)
        ciphertext = cipherCBC.encrypt(pad(cadena.encode("utf-8"), self.BLOCK_SIZE_AES))
        return ciphertext
    ...
    def descifrar(self, cifrado, IV):
        ...
        decipher_AES_CBC = AES.new(self.key, AES.MODE_CBC, IV)
        new_data = unpad(decipher_AES_CBC.decrypt(cifrado), self.BLOCK_SIZE_AES).decode("utf-8","ignore")
        return new_data
